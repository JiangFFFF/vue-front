      ...
      <section class="c-sort-box">
        ...
        <div class="mt40">
          ...
          <article class="comm-course-list" v-if="data.total>0">
            <ul class="of" id="bna">
              <li v-for="item in data.item" :key="item.id">
                <div class="cc-l-wrap">
                  ...
                  <section class="mt10 hLh20 of">
                    <span class="fr jgTag bg-green">
                      ...
                      <i class="c-fff fsize12 f-fA" v-else-if="Number(item.price)>0">{{item.price}}¥</i>
                    </span>
...
export default {
  ...
  methods:{
    //1、查询第一页数据
    initCourseFirst(){
      courseApi.getCourseList(1,8,this.searchObj).then(
        response=>{
    ...
    gotoPage(page){
      ...
      courseApi.getCourseList(page,8,this.searchObj).then(
        response=>{
          this.data=response.data.data
        }
      )
    ...
    searchOne(subjectParentId,index){
      this.AllIndex=''
      //把传递过来的index传递给oneIndex，为了active样式生效
      this.oneIndex=index
      this.twoIndex=-1
      //清空二级分类
      this.searchObj.subjectId=''
      this.subSubjectList=[]
      //把点击的一级分类id，赋值给searchObj
