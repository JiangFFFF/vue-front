      ...
      <section class="c-sort-box unBr">
        <div>
          ...
          <article v-if="data.total>0" class="i-teacher-list">
            <ul class="of">
              <li v-for="teacher in data.item" :key="teacher.id">
...
export default {
  // 采用异步调用,只会在第一次调用且只调用一次
  // paarms:相当于之前的this.$route.params.id 等价 params.id
  // asyncData({ params,error}) {
  //   return teacherApi.getTeacherList(1,8).then(
  //     response=>{
  //       return{
  ...
  methods:{
    getTeacherPage(){
      teacherApi.getTeacherList(1,8).then(
        response=>{
    ...
    gotoPage(page){
      ...
      teacherApi.getTeacherList(page,8).then(
        response=>{
          this.data=response.data.data
        }
      )
